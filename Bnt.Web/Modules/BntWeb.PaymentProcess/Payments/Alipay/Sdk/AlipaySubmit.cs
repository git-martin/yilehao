...
namespace Com.Alipay
{
    ...
    public class Submit
    {
        ...
        //商户的MD5私钥
        private readonly string _md5Key;
        //编码格式
        ...
        public Submit(AlipayConfig2 config)
        {
            _partner = config.Partner;
            _md5Key = config.Md5Key.Trim();
            _privateKey = config.PrivateKey;
            _inputCharset = config.InputCharset.Trim().ToLower();
            _signType = config.SignType;
        }
        ...
        public string BuildRequest(SortedDictionary<string, string> sParaTemp, string strMethod, string strButtonValue)
        {
            ...
            sbHtml.Append("<form id='alipaysubmit' name='alipaysubmit' action='" + GATEWAY_NEW + "_input_charset=" + _inputCharset + "' method='" + strMethod.ToLower().Trim() + "'>");
        ...
        {
            string url = GATEWAY_NEW + "service=query_timestamp&partner=" + _partner + "&_input_charset=" + _inputCharset;
            string encryptKey = "";
            ...
            encryptKey = xmlDoc.SelectSingleNode("/alipay/response/timestamp/encrypt_key").InnerText;
