...
namespace WxPayAPI
{
    public class WxPayApi
    {
        ...
        */
        public static WxPayData OrderQuery(WeiXinPayConfig weiXinPayConfig,WxPayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("appid", weiXinPayConfig.APPID);//公众账号ID
            inputObj.SetValue("mch_id", weiXinPayConfig.MCHID);//商户号
            inputObj.SetValue("nonce_str", WxPayApi.GenerateNonceStr());//随机字符串
            inputObj.SetValue("sign", inputObj.MakeSign(weiXinPayConfig.KEY));//签名
            ...
            //将xml格式的数据转化为对象以返回
            WxPayData result = new WxPayData();
            result.FromXml(response, weiXinPayConfig.KEY);
        ...
        */
        ...
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut, WeiXinPayConfig weiXinPayConfig)
        {
            if (timeOut == 0)
            {
                timeOut = 6;
            ...
            if (!inputObj.IsSet("notify_url"))
            {
                inputObj.SetValue("notify_url", WeiXinPayConfig.NOTIFY_URL);//异步通知url
            }
            ...
            //签名
            inputObj.SetValue("sign", inputObj.MakeSign(weiXinPayConfig.KEY));
            string xml = inputObj.ToXml();
